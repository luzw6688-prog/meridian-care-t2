"use strict";(()=>{var l={type:"massage_tool",name:"Acupressure Massage Stick",headline:"Apply steady pressure to hard-to-reach acupoints\u2014without tiring your fingers.",supportingCopy:"A simple acupressure tool designed for everyday relaxation routines.",originalPriceLabel:"Original price",presalePriceLabel:"Current pre-sale price",ctaLabel:"I Want This",ctaCompleteLabel:"Interest Recorded",ctaHelper:"This product is currently being prepared. Thank you for your patience."},x={a:{variant:"a",originalPrice:"$29.9",price:"$19.9"},b:{variant:"b",originalPrice:"$29.9",price:"$19.9"}},o={assignedVariant:"meridianCare.tool.priceVariant.v1",interestPrefix:"meridianCare.tool.interest.v1",analyticsEvents:"meridianCare.tool.analytics.events.v1",notificationEmailPrefix:"meridianCare.tool.notificationEmail.v1",anonymousId:"meridianCare.tool.anonymousId.v1",sessionId:"meridianCare.tool.sessionId.v1",attribution:"meridianCare.tool.attribution.v1",landingContext:"meridianCare.tool.landingContext.v1"};var H=e=>e==="a"||e==="b";function O(e){let a=new URLSearchParams(e).get("variant"),t=localStorage.getItem(o.assignedVariant),i;return H(a)?i=a:H(t)?i=t:i=crypto.getRandomValues(new Uint8Array(1))[0]<128?"a":"b",localStorage.setItem(o.assignedVariant,i),x[i]}function V(e){let a=e.price.replace(/[^0-9.]/g,"");return`${o.interestPrefix}:${ce}:${e.variant}:${a}`}var ce="massage_tool";function R(e){return localStorage.getItem(V(e))==="true"}function q(e){localStorage.setItem(V(e),"true")}var N=(e,a)=>{let t=e.getItem(a);if(t)return t;let i=crypto.randomUUID();return e.setItem(a,i),i},de=()=>window.innerWidth<768?"mobile":window.innerWidth<1024?"tablet":"desktop",D=location.protocol==="file:"||location.hostname==="localhost"||location.hostname==="127.0.0.1",k=class e{constructor(a,t){this.price=a;this.attribution=t}price;attribution;anonymousId=N(localStorage,o.anonymousId);sessionId=N(sessionStorage,o.sessionId);startedAt=performance.now();highestScrollDepth=0;setScrollDepth(a){this.highestScrollDepth=Math.max(this.highestScrollDepth,a)}track(a,t={}){let i={event_name:a,event_id:crypto.randomUUID(),anonymous_id:this.anonymousId,session_id:this.sessionId,timestamp:new Date().toISOString(),page:"/tool",product_type:l.type,price_variant:this.price.variant,displayed_price:this.price.price,source:this.attribution.source,medium:this.attribution.medium,campaign:this.attribution.campaign,content:this.attribution.content,term:this.attribution.term,referrer:this.attribution.referrer,landing_url:this.attribution.landingUrl,device_type:de(),viewport_width:window.innerWidth,scroll_depth:t.scroll_depth??this.highestScrollDepth,stay_time:Math.round(performance.now()-this.startedAt),...t},s=e.getEvents();return s.push(i),localStorage.setItem(o.analyticsEvents,JSON.stringify(s.slice(-1e3))),D&&console.info("[Meridian Tool Analytics]",i),i}static getEvents(){try{return JSON.parse(localStorage.getItem(o.analyticsEvents)??"[]")}catch{return[]}}static clearEvents(){localStorage.removeItem(o.analyticsEvents),D&&console.info("[Meridian Tool Analytics] Test events cleared.")}};window.MeridianToolAnalytics={getEvents:k.getEvents,clearEvents:k.clearEvents};var w={productMainWood:new URL("./assets/product-main-wood.jpg",location.href).href,productMainMetal:new URL("./assets/product-main-metal.jpg",location.href).href,stepFind:new URL("./assets/step-find-metal.jpg",location.href).href,stepPressure:new URL("./assets/step-pressure-metal.jpg",location.href).href,stepRoutine:new URL("./assets/step-routine-metal.jpg",location.href).href},f=e=>`<svg viewBox="0 0 32 32" aria-hidden="true">${{finger:'<path d="M9 21v-8.5a2 2 0 0 1 4 0V15m0-4.5a2 2 0 0 1 4 0V15m0-3a2 2 0 0 1 4 0v5c0 4.5-2.7 7-7.2 7H12c-2.2 0-3.8-1-5-2.8l-3-4.4a2.1 2.1 0 0 1 3.2-2.7L9 15.5"/>',pressure:'<circle cx="14" cy="15" r="8"/><path d="M14 3v8m-3-3 3 3 3-3M6 27h16"/>',guide:'<path d="M6 5.5A3.5 3.5 0 0 1 9.5 2H25v23H9.5A3.5 3.5 0 0 0 6 28.5z"/><path d="M6 5.5v23M11 8h9m-9 5h7"/>',leaf:'<path d="M25 4C14 4 7 9 7 17c0 4 3 7 7 7 8 0 11-9 11-20Z"/><path d="M5 28c3-7 8-12 15-16"/>',check:'<path d="m6 16 6 6L26 8"/>'}[e]}</svg>`;function P({location:e,complete:a,compact:t=!1}){let i=a?"cta.recorded":"cta.primary";return`
    <button
      ...
      ${a?"disabled":""}
    >
      <span class="button-icon" aria-hidden="true">${a?f("check"):""}</span>
      <span data-cta-label data-i18n="${i}">${a?l.ctaCompleteLabel:l.ctaLabel}</span>
    </button>
  `}function F(e,a=""){return`
    <div class="price-block ${a}">
      <div class="price-original">
        <span data-i18n="product.originalPriceLabel">${l.originalPriceLabel}</span>
        <del data-original-price>${e.originalPrice}</del>
      </div>
      <div class="price-current">
        <span data-i18n="product.presalePriceLabel">${l.presalePriceLabel}</span>
  ...
  `}var pe=[{icon:"finger",titleKey:"pain.1.title",copyKey:"pain.1.copy",title:"Fingers get tired during acupressure",copy:"A small tool can take the strain out of repeated pressure."},{icon:"pressure",titleKey:"pain.2.title",copyKey:"pain.2.copy",title:"It\u2019s difficult to maintain steady pressure",copy:"Rounded ends help keep your touch controlled and consistent."},{icon:"guide",titleKey:"pain.3.title",copyKey:"pain.3.copy",title:"It can be hard to know how and where to press",copy:"Simple visual routines help you get started with confidence."}],me=[{number:"01",titleKey:"benefit.1.title",copyKey:"benefit.1.copy",title:"Steady Pressure",copy:"Apply consistent pressure with less effort."},{number:"02",titleKey:"benefit.2.title",copyKey:"benefit.2.copy",title:"Easier on Your Fingers",copy:"Make everyday acupressure routines more comfortable."},{number:"03",titleKey:"benefit.3.title",copyKey:"benefit.3.copy",title:"Simple Guidance Included",copy:"Follow easy routines for common relaxation goals."}],ge=[{number:"1",titleKey:"steps.1.title",altKey:"steps.1.alt",title:"Find the suggested point",image:w.stepFind,alt:"Photo of positioning the champagne-gold metal massage stick near the shoulder"},{number:"2",titleKey:"steps.2.title",altKey:"steps.2.alt",title:"Apply gentle pressure",image:w.stepPressure,alt:"Photo of applying gentle pressure to the palm with the champagne-gold metal massage stick"},{number:"3",titleKey:"steps.3.title",altKey:"steps.3.alt",title:"Follow the guided routine",image:w.stepRoutine,alt:"Photo of the champagne-gold metal massage stick beside a guided routine on a phone"}],ye=[["details.material","Material","details.materialValue","Natural sandalwood or champagne-gold anodized aluminum alloy"],["details.size","Size","details.sizeValue","4.7 in (12 cm), compact and portable"],["details.package","Package","details.packageValue","Massage stick and quick-start guide"],["details.care","Care","details.careValue","Wipe clean and keep dry after use"]],L=[{id:"wood",className:"material-swatch-wood",titleKey:"details.woodTitle",copyKey:"details.woodCopy",altKey:"details.woodVisualAlt",selectedKey:"details.woodSelected",title:"Natural Sandalwood",copy:"A warm, smoothly polished wood finish with natural grain variation.",image:w.productMainWood,alt:"Natural sandalwood Acupressure Massage Stick on a warm neutral surface"},{id:"metal",className:"material-swatch-metal",titleKey:"details.metalTitle",copyKey:"details.metalCopy",altKey:"details.metalVisualAlt",selectedKey:"details.metalSelected",title:"Champagne Metal",copy:"A satin anodized aluminum-alloy finish with a smooth, low-glare surface.",image:w.productMainMetal,alt:"Champagne-gold metal Acupressure Massage Stick on a warm neutral surface"}];function U(e,a){return`
    <header class="brand-header">
      <a class="brand-lockup" href="../" aria-label="Meridian Care home" data-i18n-aria-label="nav.homeLabel">
        <span class="brand-symbol" aria-hidden="true">${f("leaf")}</span>
    ...
    <main id="main-content">
      <section class="hero section-shell" data-track-section="hero">
        <div class="hero-copy">
          ...
          <h1 data-i18n="product.name">${l.name}</h1>
          <p class="hero-headline" data-i18n="product.headline">${l.headline}</p>
          <p class="hero-support" data-i18n="product.support">${l.supportingCopy}</p>
        </div>

        ${ue()}

        <div class="hero-conversion">
          ${F(e,"price-block-hero")}
          ${P({location:"hero",complete:a})}
          <p class="cta-helper" data-i18n="cta.helper">${l.ctaHelper}</p>
        </div>

        <ul class="trust-list" aria-label="Product highlights" data-i18n-aria-label="trust.label">
          <li>${f("check")}<span data-i18n="trust.easy">Easy to use</span></li>
          <li>${f("check")}<span data-i18n="trust.gentle">Gentle daily care</span></li>
          <li>${f("check")}<span data-i18n="trust.guided">Guided routines included</span></li>
        </ul>
      </section>
        ...
        <div class="pain-grid">
          ${pe.map(t=>`
                <article class="pain-card reveal">
                  <span class="line-icon">${f(t.icon)}</span>
                  <div>
      ...
      <section class="content-section section-shell details-layout" data-track-section="product_details">
        ...
        <div class="details-copy">
          ...
          >
            ${L.map((t,i)=>`
                  <button
                    class="material-option${i===0?" is-selected":""}"
                    type="button"
                    data-material-option="${t.id}"
                    data-material-image="${t.image}"
                    data-material-alt-key="${t.altKey}"
                    data-material-title-key="${t.titleKey}"
                    data-material-selected-key="${t.selectedKey}"
                  ...
                  >
                    ...
                    <span class="material-option-check" aria-hidden="true">${f("check")}</span>
                  </button>
                `).join("")}
          </div>
          <div class="sr-only" aria-live="polite" data-material-live></div>
          <dl class="detail-list reveal">
            ${ye.map(([t,i,s,n])=>`<div><dt data-i18n="${t}">${i}</dt><dd data-i18n="${s}">${n}</dd></div>`).join("")}
          </dl>
        </div>
      </section>

      <section class="section-shell safety-notice reveal" data-track-section="safety">
        <span class="line-icon">${f("leaf")}</span>
        <div>
      ...
      <section class="bottom-conversion" data-bottom-conversion data-track-section="bottom_cta">
        <div class="section-shell bottom-conversion-inner reveal">
          <div>
            ...
            <h2 data-i18n="product.name">${l.name}</h2>
          </div>
          <div class="bottom-action">
            ${F(e,"price-block-light")}
            ${P({location:"bottom",complete:a})}
            <p class="cta-helper cta-helper-light" data-i18n="cta.helper">${l.ctaHelper}</p>
          </div>
        </div>
      </section>
    </main>
